/**
 * Validation du moteur v5 — à lancer avec :  npm run validate:v5
 *
 ...
 * officielles de la banque centrale redonne les chiffres publiés, et que deux scénarios
 * qui se recouvrent donnent exactement les mêmes valeurs.
 *
...
import {
  historicalQuartersCount, historicalShocks, historicalPolicyRate, historicalReserveRequirement,
...
/** Régime 1 : le jeu tel qu'il tourne, joueur = décisions officielles. */
function modeJeu(id: ScenarioId) {
  ...
  for (let q = 0; q < historicalQuartersCount(id); q++) {
    const rate = historicalPolicyRate(id, q) ?? last
    last = rate
...
console.log('\n─── RÉGIME 1 — mode jeu, joueur = décisions officielles de la banque centrale ───')
