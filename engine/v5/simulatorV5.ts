 ...
 *    (upi/uy de covid_real_data.json) pour reproduire l'histoire 2020-2025.
 */
...
export interface StepV5Options {
   ...
   * Défaut → sensibilités de jeu (κ=0,10, σ=0,32), qui rendent les décisions visibles
...
export function stepV5(
  ...
  let liquidityNeed = rawLiquidityNeed + action.fxInterventionBnMad - action.emergencyLendingBnMad
  ...
  const supplyShock = real
    ...
    : activeShocks
        ...
        + activeShocks
            ...
            .reduce((acc, s) => acc + PARAMS_V5.phillips.const * 0, 0) // agri passe par gamma dans le v1 ; ici via u^π
