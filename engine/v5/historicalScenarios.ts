 ...
 *
 * Quatre scénarios de la plateforme rejouent une période réelle : le joueur qui
 * reproduit les décisions officielles de la banque centrale retrouve les séries du HCP.
 *
 *   covid2020      2020T3 → 2025T2   (20 trimestres)
 *   flexibilite    2018T2 → 2023T1   (20 trimestres)
 *   crisis2008     2008T2 → 2013T1   (20 trimestres)
 *   inflation2022  2022T2 → 2027T1   (20 trimestres)
 *
 ...
 * comme constante propre à ce trimestre.
 *
 ...
 * identiques à ceux d'avant l'ajout des trois autres crises.
 */
...
import {
  COVID_REAL_QUARTERS_COUNT, covidRealPolicyRate, covidRealHcp,
...
/** Taux directeur réellement décidé ce trimestre-là (repère historique pour le joueur). */
export function historicalPolicyRate(id: ScenarioId, idx: number): number | null {
  if (id === 'covid2020') return covidRealPolicyRate(idx)
  const d = get(id)
  const k = d?.quarters[idx]
  return d && k ? (d.policyRate[k] ?? null) : null
}

/** Réserve obligatoire réellement en vigueur ce trimestre-là (en %). */
export function historicalReserveRequirement(id: ScenarioId, idx: number): number | null {
